...
The original client/server version of VoicePushPTT. A small Node server hands
out channels and decides who holds the floor; audio still goes directly between
browsers over WebRTC. Join a channel, hold the button (or <kbd>Space</kbd>),
talk.

For most people the [desktop app](electron/README.md) or the
[single-file version](serverless/README.md) is easier, because neither needs a
server.
...
- This version has no message history. The single-file and desktop versions
  keep local history.
